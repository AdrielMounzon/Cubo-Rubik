...
class StatesSpace:
    ...
    def add_state(self, value):
        if value not in self.space:
            self.space[value] = State(value)    #Validar si la combinación del cubo no estaba antes
...
class Searcher:
    ...
    def weighted_search(self, intial_value, goal_value, frontier):
        initial_state = self.space.get_state(intial_value)  #Carga cubo inicial
        initial_state.set_cost(0)
        frontier.put((initial_state, 0))

        while not frontier.empty():
            current_state, current_cost = frontier.get()    #Obtención dato frontier
            ...
            for action in current_state.actions:    #Cambiar lógica para aplicar la acción al cubo o hacerlo antes
                self.space.add_state(action[0])
                next_state = self.space.get_state(action)   #Ejecutar el comando
                ...
                if not next_state.was_reached() or action_cost < next_state.cost:   #Validación costo
                    print("Entra")
                    ...
                    frontier.put((next_state, action_cost)) #Colocación siguiente dato en frontier
        return []
